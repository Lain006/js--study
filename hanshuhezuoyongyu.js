...
function introduce(name, age) {
    console.log(`我叫\( {name}, 今年 \){age}岁`); // 模板字符串，用于动态展示数据
}
introduce("小王", 18);
...
function showWebsiteName() {
    console.log(websiteName, '函数外部都可以访问');
}
showWebsiteName();
// 块级作用域：let 和 const 声明的变量，只能在当前代码块内有效
if (true) {
    let blockLet = "let 在代码块内有效";
    const blockConst = "const 在代码块内有效";
    console.log(blockLet, blockConst, '代码块内可以访问');
}
// console.log(blockLet, blockConst, '代码块外无法访问'); // 这行会报错
